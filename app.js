...
function renderProducts() {
  ...
  grid.innerHTML = filtered.map(product => `
    <div class="card">
      <div class="productImage">${product.icon}</div>
      <h3>${product.name}</h3>
...
document.getElementById("checkoutForm").addEventListener("submit", function(e) {
  ...
  const orderList = cart.map(item => `- ${item.name} x ${item.quantity} = ${formatNaira(item.price * item.quantity)}`).join("%0A");

  const message = `Hello Biserry Groceries,%0A%0AI want to place an order.%0A%0AName: ${name}%0APhone: ${phone}%0AOrder Type: ${fulfillment}%0APayment Method: ${paymentMethod}%0AAddress/Note: ${address}%0A%0AItems:%0A${orderList}%0A%0ATotal: ${formatNaira(total)}`;
