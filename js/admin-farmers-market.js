    ...
    table.innerHTML = snap.docs.map(docSnap => {
      ...
      return `
        <tr>
          ...
          <td>${r.shoppingList || ""}</td>
          <td>${r.budgetRange || ""}</td>
          <td>${r.preferredDeliveryDate || ""}</td>
          <td>
            <select onchange="updateRequestStatus('${docSnap.id}', this.value)">
              ${["New","Sourcing","Confirmed","Delivered","Cancelled"].map(s => `<option value="${s}" ${r.status === s ? "selected" : ""}>${s}</option>`).join("")}
            </select>
          </td>
          <td>
            <a class="btn small" target="_blank" href="https://wa.me/${String(r.customerPhone || '').replace(/\D/g,'')}">WhatsApp</a>
          </td>
        </tr>
      `;
    }).join("");
...
window.updateRequestStatus = async function(id, status) {
  try {
    await updateDoc(doc(db, "farmers_market_requests", id), { status });
  } catch (error) {
