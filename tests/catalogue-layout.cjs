...
(async()=>{
 ...
 const browser=await chromium.launch({headless:true});const page=await browser.newPage({serviceWorkers:'block'});
 ...
 for(const file of ['shop.html','index.html']) for(const [width,expected] of [[390,2],[1024,4],[1366,5],[1600,6]]){
  ...
  if(file==='shop.html'&&width===1366){await page.locator('#shop').screenshot({path:'/tmp/biserry-desktop.png'});await page.locator('.variantBox select').first().selectOption('600g');await page.locator('.addBtn').first().click();const cart=await page.evaluate(()=>JSON.parse(localStorage.getItem('biserryCart')));assert.equal(cart[0].variantId,'600g');assert.equal(cart[0].price,6000);}
 }
